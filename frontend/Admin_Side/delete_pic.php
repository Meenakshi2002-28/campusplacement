...
if ($user_id) {
    ...
    $sql = "SELECT photo FROM admin WHERE user_id = ?";
    ...
    if ($current_picture) {
        ...
        $sql = "UPDATE admin SET photo = NULL WHERE user_id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("s", $user_id);
        $stmt->execute();
        $stmt->close();
        ...
        header("Location: profile_admin.php?user_id=" . urlencode($user_id)); // Redirect to profile page
        exit();
    } else {
